...
$ python3 generate_namelists.py [--sp/--mp] [--stoch/--nostoch] [--onlydo x]
                                [--overwrite] [--donotenforce] [--final] [--year xxxx]
                                [--casestr YYYYMMDD]
...
Option --year will run the NEWS-e set up for that year. So 2016 uses Thompson.
...
parser.add_argument('--vse',action='store_false')
#parser.add_argument('--final',action='store_true')
parser.add_argument('--year',type=int,default=False)
...
# path_to_namelist_template = '/home/john.lawson/VSE_reso/pycode/namelists/namelist.input.template'
...
for ex in ("MP","MP-Stoch"):
    for n in ensnums:
        ...
        bl_mems = [x+(6*n) for x in [1,2] for n in range(5)]

        # pdb.set_trace()
        if n in bl_mems: # member 1,2,7,8...
            NL[ex][n]['bl_pbl_physics'] = 1
            NL[ex][n]['sf_sfclay_physics'] = 1
        elif n in [a+2 for a in bl_mems]: # member 3,4,9,10...
            NL[ex][n]['bl_pbl_physics'] = 2
            NL[ex][n]['sf_sfclay_physics'] = 2
        else: # member 5,6,11,12...
            NL[ex][n]['bl_pbl_physics'] = 5
            NL[ex][n]['sf_sfclay_physics'] = 5

        # NL[ex][n]['cu_physics'] = 0
...
for ex in experiments:
    for n in ensnums:
        NL[ex][n]['input_from_file'] = ('.true.','.false.')
        NL[ex][n]['fine_input_stream'] = 0
        NL[ex][n]['history_interval'] = 5
        NL[ex][n]['frames_per_outfile'] = 999
        NL[ex][n]['cycling'] = ('.false.',)
        NL[ex][n]['all_ic_times'] = ('.false.',)
        # NL[ex][n]['time_step'] = 9
        NL[ex][n]['max_dom'] = (2,)
        NL[ex][n]['e_we'] = (250,322)
        NL[ex][n]['e_sn'] = (250,322)
        NL[ex][n]['e_vert'] = 51
        NL[ex][n]['dx'] = (3000.0,1000.0)
        NL[ex][n]['parent_id'] = (0,1)
        NL[ex][n]['i_parent_start'] = (0,61)
        NL[ex][n]['j_parent_start'] = (0,61)
        NL[ex][n]['parent_grid_ratio'] = (1,3)
        NL[ex][n]['parent_time_step_ratio'] = (1,3)
        NL[ex][n]['numtiles'] = (1,)
        NL[ex][n]['nproc_x'] = (-1,)
        NL[ex][n]['nproc_y'] = (-1,)
        NL[ex][n]['reorder_mesh'] = ('.false.',)
        NL[ex][n]['radt'] = 1
        # NL[ex][n]['
